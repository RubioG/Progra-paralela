...
int main(int argc, char* argv[])
{
    ...
    if(my_rank == 0){
        size_t temp_threads = 0;
        size_t temp_primes = 0;
        for(int index = 1; index < process_count; ++index){
            MPI_Recv(&temp_primes, 1, MPI_UNSIGNED_LONG_LONG, index, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv(&temp_threads, 1, MPI_UNSIGNED_LONG_LONG, index, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            cant_primes+= temp_primes;
            cant_threads+= temp_threads;
        }
        printf("\tProcess %zu on %s found %zu primes in range [%zu,%zu[ in %zu s with %zu threads\n", my_rank ,hostname, cant_primes , range_x, range_y , 0, cant_threads);
    }
    else{
        MPI_Send(&range_x, 1 , MPI_UNSIGNED_LONG_LONG, 0, 0,  MPI_COMM_WORLD);
        MPI_Send(&range_y, 1 , MPI_UNSIGNED_LONG_LONG, 0, 0,  MPI_COMM_WORLD);
    }
